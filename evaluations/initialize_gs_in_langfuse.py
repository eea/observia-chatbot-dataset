...
json_files = [
    "dataset/Copernicus Land (CLMS)-goldenset.json",
        ...
        for index, row in sheet.iterrows():
            ...
            for col in columns_to_check:
                if not pd.isna(row.get(col)):
                    if (
                        not row.get(
                            "Evaluation", "3: good question - good answer")
...
sheets = load_dataset_from_excel(
    gs_excel=excel_file, all_contexts=all_contexts)
